...
-subj "/C=NG/ST=Enugu/L=Enugu/O=onyeka/OU=devops/CN=$(curl -s http://169.254.169.254/latest/meta-data/local-hostname)"


sudo sed -i 's/localhost.crt/onyi.crt/g'  /etc/httpd/conf.d/ssl.conf

sudo sed -i 's/localhost.key/onyi.key/g'  /etc/httpd/conf.d/ssl.conf